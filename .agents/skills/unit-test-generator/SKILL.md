...
description: Generates comprehensive unit tests with AAA pattern (Arrange-Act-Assert), edge cases, error scenarios, and coverage analysis. Creates test files matching source structure with complete test suites. Use for "unit testing", "test generation", "Jest tests","Vitest tests" or "test coverage".
---
...
| Spy on object | `vi.spyOn()` | `jest.spyOn()` |
| Mock module | `vi.mock()` | `jest.mock()` |
| Global stub | `vi.stubGlobal()` | `Object.defineProperty(global/window)` |
